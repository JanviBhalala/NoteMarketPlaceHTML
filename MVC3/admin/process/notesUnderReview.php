...
ob_start();
$select="SELECT users.user_id,note.note_id,note.title,category.name,note.created_date,status.status,note.status_id,users.fname,users.lname FROM note JOIN category ON note.category_id=category.category_id JOIN status ON note.status_id= status.status_id JOIN users ON note.user_id=users.user_id WHERE ";
$where="note.status_id IN (2,5) AND note.is_active=1 ";
$order_by=" ORDER BY note.created_date DESC";
...
if( !empty($_POST)){
    ...
    if($value !=''){
        if($name !='searchBytxt'){
			...
			$where .="AND (title LIKE '".$value1."' OR name LIKE '".$value1."' OR users.fname LIKE '".$value1."' OR users.lname LIKE '".$value1."'  OR note.created_date LIKE '".$value1."') ";	
        }
